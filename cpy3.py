#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""cpy3.py —— c-like-python (.cpy) 转换并运行器（{ 自动换行版）。

与 cpy.py 的唯一区别在于普通 { 的处理：
    cpy.py  ：{ 只把缩进计数 +1，不换行（块头需写成 if a:; { ... }）。
    cpy3.py ：{ 把缩进计数 +1，并换行；但若此时已处于行首（例如前面的 ; 刚换过行），
              则只加缩进、不再换行，以免产生多余空白行。
因此可以直接写 if a: { ... }，{ 会自动产生 if a: 之后的换行。
该规则向后兼容老语法：if a:; { ... } 仍然得到正确结果、不产生空行。

注意（语言约定）：} 不结束语句、也不换行。每一条语句（包括 } 之前的最后一条）
都必须以 ; 结尾，例如 { a(); b(); } —— 最后的 b() 后面同样要有 ;。

用法：
    python cpy3.py abc.cpy [args...]

把 abc.cpy 转换成同目录下的 abc.py，然后运行 abc.py。
多余的命令行参数会原样透传给被运行的脚本。

语言规则（见《项目目标.txt》，本实现仅改动 { ）：
    换行     -> 行内时等同空格，行首时忽略（吞掉）
    ;        -> 换行（必然换行，即使在行首），并进入行首状态
    {        -> 缩进计数 +1，并换行（行首时只加缩进、不换行）
    }        -> 缩进计数 -1（不输出字符、不换行）
    ${ $} $; -> 字面量 { } ;
    $$       -> 字面量 $
...
def convert(src: str) -> str:
    """把 c-like-python 源码转换成普通 Python 源码（{ 自动换行版）。"""
    out = []
    indent = 0
...
def main(argv=None) -> int:
    ...
    if len(argv) < 2:
        print("用法：python cpy3.py abc.cpy [args...]", file=sys.stderr)
        return 2
